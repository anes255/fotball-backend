...
app.post('/api/tournament-winner', auth, async (req, res) => { try { const {tournament_id,team_id}=req.body; const s=await pool.query(`SELECT id FROM matches WHERE tournament_id=$1 AND status IN ('live','completed') LIMIT 1`,[tournament_id]); if(s.rows.length>0) return res.status(400).json({error:'Tournoi déjà commencé'}); res.json((await pool.query('INSERT INTO tournament_winner_predictions(user_id,tournament_id,team_id) VALUES($1,$2,$3) ON CONFLICT(user_id,tournament_id) DO UPDATE SET team_id=$3 RETURNING *',[req.userId,tournament_id,team_id])).rows[0]); } catch(e) { res.status(500).json({error:'Erreur'}); }});

app.get('/api/tournaments/:id/started', async (req, res) => { try { const s=await pool.query(`SELECT id FROM matches WHERE tournament_id=$1 AND status IN ('live','completed') LIMIT 1`,[req.params.id]); res.json({started:s.rows.length>0}); } catch(e) { res.status(500).json({error:'Erreur'}); }});
