...
const initDB = async () => {
  ...
  const defaults = [['primary_color','#6366f1'],['accent_color','#8b5cf6'],['bg_color','#0f172a'],['card_color','#1e293b'],['header_name','Prediction World'],['header_logo',''],['home_name','Prediction World'],['home_logo',''],['site_name','Prediction World'],['site_logo','']];
...
app.post('/api/tournaments', auth, adminAuth, async (req, res) => { try { const {name,description,start_date,end_date,logo_url,is_active,format,max_teams}=req.body; const t=(await pool.query('INSERT INTO tournaments(name,description,start_date,end_date,logo_url,is_active,format,max_teams) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',[name,description,start_date,end_date,logo_url,is_active!==false,format||'groups_4',max_teams||32])).rows[0]; const gr=(await pool.query('SELECT rule_type,points FROM scoring_rules')).rows; for(const r of gr) await pool.query('INSERT INTO tournament_scoring_rules(tournament_id,rule_type,points) VALUES($1,$2,$3) ON CONFLICT DO NOTHING',[t.id,r.rule_type,r.points]); res.json(t); } catch(e) { console.error(e); res.status(500).json({error:'Erreur'}); }});
app.put('/api/tournaments/:id', auth, adminAuth, async (req, res) => { try { const {name,description,start_date,end_date,logo_url,is_active,format,max_teams}=req.body; res.json((await pool.query('UPDATE tournaments SET name=$1,description=$2,start_date=$3,end_date=$4,logo_url=$5,is_active=$6,format=$7,max_teams=$8 WHERE id=$9 RETURNING *',[name,description,start_date,end_date,logo_url,is_active,format,max_teams||32,req.params.id])).rows[0]); } catch(e) { res.status(500).json({error:'Erreur'}); }});
...
app.post('/api/tournament-winner', auth, async (req, res) => { try { const {tournament_id,team_id}=req.body; const s=await pool.query(`SELECT id,(match_date<=NOW()) as tp,(status IN ('live','completed')) as ss FROM matches WHERE tournament_id=$1 ORDER BY match_date LIMIT 1`,[tournament_id]); if(s.rows[0]&&(s.rows[0].tp||s.rows[0].ss)) return res.status(400).json({error:'Tournoi déjà commencé'}); res.json((await pool.query('INSERT INTO tournament_winner_predictions(user_id,tournament_id,team_id) VALUES($1,$2,$3) ON CONFLICT(user_id,tournament_id) DO UPDATE SET team_id=$3 RETURNING *',[req.userId,tournament_id,team_id])).rows[0]); } catch(e) { res.status(500).json({error:'Erreur'}); }});

app.get('/api/tournaments/:id/started', async (req, res) => { try { const s=await pool.query(`SELECT id FROM matches WHERE tournament_id=$1 AND (status IN ('live','completed') OR match_date<=NOW()) LIMIT 1`,[req.params.id]); res.json({started:s.rows.length>0}); } catch(e) { res.status(500).json({error:'Erreur'}); }});

// Leaderboard - computed from predictions
app.get('/api/leaderboard', async (req, res) => { try { res.json((await pool.query(`SELECT u.id,u.name,COALESCE((SELECT SUM(p.points_earned) FROM predictions p JOIN matches m ON p.match_id=m.id WHERE p.user_id=u.id AND m.status='completed'),0)+COALESCE((SELECT SUM(twp.points_earned) FROM tournament_winner_predictions twp WHERE twp.user_id=u.id),0)+COALESCE((SELECT SUM(pp.points_earned) FROM player_predictions pp WHERE pp.user_id=u.id),0) AS total_points,(SELECT COUNT(*) FROM predictions WHERE user_id=u.id) as total_predictions FROM users u ORDER BY total_points DESC,u.name`)).rows); } catch(e) { console.error(e); res.status(500).json({error:'Erreur'}); }});

// Per-tournament leaderboard
app.get('/api/leaderboard/tournament/:id', async (req, res) => { try { const tid=req.params.id; res.json((await pool.query(`SELECT u.id,u.name,COALESCE((SELECT SUM(p.points_earned) FROM predictions p JOIN matches m ON p.match_id=m.id WHERE p.user_id=u.id AND m.tournament_id=$1 AND m.status='completed'),0)+COALESCE((SELECT SUM(twp.points_earned) FROM tournament_winner_predictions twp WHERE twp.user_id=u.id AND twp.tournament_id=$1),0)+COALESCE((SELECT SUM(pp.points_earned) FROM player_predictions pp WHERE pp.user_id=u.id AND pp.tournament_id=$1),0) AS total_points,(SELECT COUNT(*) FROM predictions p2 JOIN matches m2 ON p2.match_id=m2.id WHERE p2.user_id=u.id AND m2.tournament_id=$1) as total_predictions FROM users u HAVING COALESCE((SELECT SUM(p.points_earned) FROM predictions p JOIN matches m ON p.match_id=m.id WHERE p.user_id=u.id AND m.tournament_id=$1 AND m.status='completed'),0)+COALESCE((SELECT SUM(twp.points_earned) FROM tournament_winner_predictions twp WHERE twp.user_id=u.id AND twp.tournament_id=$1),0)+COALESCE((SELECT SUM(pp.points_earned) FROM player_predictions pp WHERE pp.user_id=u.id AND pp.tournament_id=$1),0)>0 OR (SELECT COUNT(*) FROM predictions p2 JOIN matches m2 ON p2.match_id=m2.id WHERE p2.user_id=u.id AND m2.tournament_id=$1)>0 ORDER BY total_points DESC,u.name`,[tid])).rows); } catch(e) { console.error(e); res.status(500).json({error:'Erreur'}); }});
